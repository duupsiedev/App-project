(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&i(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();const Z={dashboard:["Overview","A local workflow preview for email triage, summaries, routing suggestions, and draft preparation."],import:["Setup preview","Preview how a future Microsoft 365 connection could import mailbox structure and workflow patterns."],triage:["Inbox triage","Review AI-classified messages before any action is taken."],rules:["Rules","Approve or adjust local rule previews. They do not affect a real mailbox."],drafts:["Drafts","Review prepared replies and mark them ready for a person to send."],admin:["Admin","Manage local workspace preferences and preview future integration safeguards."]};function r(a=""){return String(a).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function S(a=[]){return a.map(e=>r(e)).join(", ")}const Se=["Triage","Urgent emails","Drafts needing approval","Invoices","Generate digest","Create invoice rule","Reset demo data"];function De({assistantOpen:a,assistantMessages:e,assistantBusy:t}){const i=e.length?e:[{id:"assistant-loading",role:"assistant",text:"Loading assistant history..."}];return`
...
`;function ae(a,e){s.busy[a]=e,v()}async function g(a,e,t){try{ae(a,!0),await e(),t&&P(t)}catch(i){P(i.message||"Something went wrong in the mock workflow.",!0)}finally{ae(a,!1)}}function u(a){return!!s.busy[a]}function P(a,e=!1){const t=document.querySelector("#toast");t.textContent=a,t.classList.toggle("error",e),t.classList.add("show"),window.clearTimeout(t.dataset.timer),t.dataset.timer=window.setTimeout(()=>t.classList.remove("show"),2400)}function T(a,e={}){if(!ht.has(a))throw new Error("That Courio section is unavailable.");const t=s.tab;t==="admin"&&a!=="admin"&&(s.settingsForm=null),a==="admin"&&t!=="admin"&&(s.settingsForm={...s.settings}),s.tab=a,e.triageFilter&&(s.triageFilter=e.triageFilter),e.draftFilter&&(s.draftFilter=e.draftFilter),e.closeDrawers!==!1&&(s.selectedEmail=null,s.selectedCategory=null,s.selectedDraft=null,s.selectedRule=null,s.selectedEmployee=null,s.summary="",s.showExplanation=!1),v()}async function G(a=(e=>(e=s.selectedEmail)==null?void 0:e.id)()){if(s.emails=await A(),a){const t=s.emails.find(i=>i.id===a);s.selectedEmail=t?{...t,messages:t.thread}:null}}async function se(a=(e=>(e=s.selectedCategory)==null?void 0:e.id)()){const[t,i,n,o,l,h]=await Promise.all([ue(),A(),M(),F(),pe(),x()]);s.categories=t,s.emails=i,s.rules=n,s.digest=o,s.setupPreview=l,s.activity=h,s.selectedCategory=a&&t.find(D=>D.id===a)||null}function bt(a){return!!(a!=null&&a.isReadyForHumanSend)}function J(){return s.settings.mode==="Advanced"}function fe(a=""){const e=s.categories.filter(i=>i.active),t=a?s.categories.find(i=>i.name===a):null;return t&&!e.some(i=>i.id===t.id)?[...e,t]:e}function $t(a,e){var t;return((t=s.categories.find(i=>i.id===a))==null?void 0:t.name)||e}function ve(){return s.settings.allowLowRiskBulkApproval!=="No"}function B(){return s.settingsForm||{...s.settings}}function Et(){return B().mode==="Advanced"}async function St(){var a;try{const[e,t,i,n,o,l,h,D,y,f]=await Promise.all([A(),ue(),W(),M(),L(),Xe(),F(),vt(),x(),pe()]);s.emails=e,s.categories=t,s.employees=i,s.rules=n,s.drafts=o,s.settings={...s.settings,...l},s.settingsForm=null,s.digest=h,s.assistantMessages=D,s.activity=y,s.setupPreview=f,s.selectedSetupMailboxId=((a=f.mailboxes[0])==null?void 0:a.id)||""}catch(e){P(e.message||"Could not load mock data.",!0)}finally{s.loading.emails=!1,s.loading.rules=!1,s.loading.drafts=!1,v()}}function v(){document.querySelector("#pageTitle").textContent=Z[s.tab][0],document.querySelector("#pageSubtitle").textContent=Z[s.tab][1],document.querySelectorAll(".section").forEach(a=>{a.classList.toggle("active",a.id===s.tab)}),document.querySelectorAll(".nav button").forEach(a=>{a.classList.toggle("active",a.dataset.tab===s.tab)}),Dt(),we(),kt(),ye(),Lt(),_t(),At(),Rt(),Nt()}function Dt(){const a=s.emails.filter(t=>t.status!=="Done").length,e=s.digest;document.querySelector("#dashboard").innerHTML=`
    <div class="grid cols-3">
      ...
      <div class="panel metric">
        ...
        <div class="value">${s.drafts.filter(bt).length||0}</div>
        ...
        <tbody>
          ${t.map(n=>{var o;return`
            <tr>
              ...
              <td>
                <span class="badge ${V(n.category)}">${r(n.category)}</span><br>
  ...
  `}function At(){const a=document.querySelector("#drawerRoot");if(s.selectedCategory){Mt(a);return}if(s.selectedEmployee){Ft(a);return}if(s.selectedRule){Tt(a);return}if(s.selectedDraft){It(a);return}if(!s.selectedEmail){a.innerHTML="";return}const e=s.selectedEmail,t=e.status==="Done",i=s.employees.find(o=>o.id===e.assignedTo),n=fe(e.category);a.innerHTML=`
    <div class="drawer-backdrop" data-close-drawer></div>
    <aside class="review-drawer" aria-label="Email review">
      <div class="drawer-header">
        <div>
          <div class="badge ${V(e.category)}">${r(e.category)}</div>
          <h2>${r(e.subject)}</h2>
  ...
  `}function Rt(){const a=document.querySelector("#modalRoot");if(!s.confirmDialog){a.innerHTML="";return}const e=s.confirmDialog;a.innerHTML=`
  ...
  `}async function Ct(a){const e=await at(a);s.drafts=await L(),await G(a),s.selectedDraft=await q(e.id),s.selectedEmail=null,s.selectedRule=null,s.selectedEmployee=null,s.selectedCategory=null}async function xt(a){var e;await it(a),s.drafts=await L(),s.emails=await A(),s.activity=await x(),s.digest=await F(),s.selectedDraftIds=s.selectedDraftIds.filter(t=>t!==a),((e=s.selectedDraft)==null?void 0:e.id)===a&&(s.selectedDraft=await q(a))}function It(a){const e=s.selectedDraft,t=e.sourceEmail||{},i=t.status==="Done";a.innerHTML=`
    <div class="drawer-backdrop" data-close-drawer></div>
  ...
  `}function Tt(a){var i;const e=s.selectedRule,t=fe(e.category);a.innerHTML=`
    <div class="drawer-backdrop" data-close-drawer></div>
  ...
  `}function Mt(a){const e=s.selectedCategory,t=e.id==="new",i=[["default","Default"],["urgent","Red / urgent"],["invoice","Green"],["lead","Blue"],["pending","Amber"]];a.innerHTML=`
    <div class="drawer-backdrop" data-close-drawer></div>
    <aside class="review-drawer" aria-label="${t?"Add category":"Edit category"}">
      <div class="drawer-header">
        <div>
          <div class="badge ${V(e.name)}">${e.active===!1?"Archived":"Active"}</div>
  ...
  `}function Ft(a){const e=s.selectedEmployee,t=e.id==="new";a.innerHTML=`
    <div class="drawer-backdrop" data-close-drawer></div>
      ...
      <div class="panel">
        ...
        ${s.categories.length===0?'<div class="empty-state">No categories yet. Add one to make triage choices available.</div>':`<table class="table">
              ...
              <tbody>
                ${s.categories.map(e=>`
                  <tr>
                    <td><span class="badge ${V(e.name)}">${r(e.name)}</span>${e.system?"<br><small>System default</small>":""}</td>
  ...
  `}function V(a){const e=s.categories.find(t=>t.name===a);return e!=null&&e.color&&e.color!=="default"?e.color:a==="Urgent"||a==="Client complaint"?"urgent":a==="Accounting"||a==="Documents"||a==="Missing documents"?"invoice":a==="Sales"?"lead":""}document.addEventListener("click",async a=>{const e=a.target.closest("button");if(e){if(e.dataset.assistantToggle!==void 0){s.assistantOpen=!s.assistantOpen,v();return}if(e.dataset.assistantCommand){await he(e.dataset.assistantCommand);return}if(e.dataset.confirmCancel!==void 0){s.confirmDialog=null,v();return}if(e.dataset.confirmPrimary!==void 0){const t=s.confirmDialog;if(s.confirmDialog=null,(t==null?void 0:t.type)==="reset-demo"){await g("reset-demo",async()=>{await et(),window.location.reload()});return}if((t==null?void 0:t.type)==="delete-rule"){await g(`delete-rule-${t.ruleId}`,async()=>{await rt(t.ruleId),s.rules=await M(),s.activity=await x(),s.selectedRule=null},"Rule deleted locally.");return}if((t==null?void 0:t.type)==="delete-employee"){await g(`delete-employee-${t.employeeId}`,async()=>{await ft(t.employeeId),s.employees=await W(),s.emails=await A(),s.activity=await x(),s.selectedEmployee=null},"Employee removed and assigned emails returned to Unassigned.");return}}if(e.dataset.tab&&T(e.dataset.tab),e.dataset.tabTarget&&T(e.dataset.tabTarget),e.dataset.setupMailbox&&(s.selectedSetupMailboxId=e.dataset.setupMailbox,we()),e.dataset.triageFilter&&(s.triageFilter=e.dataset.triageFilter,v()),e.dataset.draftFilter&&(s.draftFilter=e.dataset.draftFilter,v()),e.dataset.action==="digest"&&await g("digest",async()=>{s.digest=await F()},"Morning digest regenerated from local demo data."),e.dataset.reviewEmail){const t=e.dataset.reviewEmail;await g(`review-${t}`,async()=>{s.selectedEmail=await me(t),s.emails=await A(),s.selectedDraft=null,s.selectedRule=null,s.selectedEmployee=null,s.summary="",s.showExplanation=!1},"Message thread opened.")}if(e.dataset.reviewDraft){const t=e.dataset.reviewDraft;await g(`review-draft-${t}`,async()=>{s.selectedDraft=await q(t),s.selectedEmail=null,s.selectedRule=null,s.selectedEmployee=null,s.selectedCategory=null,s.summary="",s.showExplanation=!1},"Draft opened for review.")}if(e.dataset.openEmailDraft){const t=e.dataset.openEmailDraft;await g(`open-email-draft-${t}`,async()=>{s.selectedDraft=await Je(t),s.selectedEmail=null,s.selectedRule=null,s.selectedEmployee=null,s.selectedCategory=null,s.summary="",s.showExplanation=!1},"Draft opened for editing.")}if(e.dataset.closeDrawer!==void 0&&(s.selectedEmail=null,s.selectedDraft=null,s.selectedRule=null,s.selectedEmployee=null,s.selectedCategory=null,s.summary="",s.showExplanation=!1,v()),e.dataset.toggleExplanation!==void 0&&(s.showExplanation=!s.showExplanation,v()),e.dataset.summaryEmail){const t=e.dataset.summaryEmail;await g(`summary-${t}`,async()=>{s.summary=await tt(t)},"Thread summary generated.")}if(e.dataset.generateDraft){const t=e.dataset.generateDraft;await g(`draft-${t}`,async()=>{await Ct(t)},"Draft opened. Existing edits were preserved.")}if(e.dataset.saveDraft){const t=e.dataset.saveDraft,i=document.querySelector("[data-draft-editor]");await g(`save-${t}`,async()=>{var n;await st(t,i.value),s.drafts=await L(),s.emails=await A(),((n=s.selectedDraft)==null?void 0:n.id)===t&&(s.selectedDraft=await q(t))},"Draft saved locally.")}if(e.dataset.toggleRule){const t=e.dataset.toggleRule;await g(`rule-${t}`,async()=>{await te(t),s.rules=await M()},"Rule preview state updated.")}if(e.dataset.approveRule){const t=e.dataset.approveRule;await g(`approve-rule-${t}`,async()=>{s.rules.find(n=>n.id===t).on||await te(t),s.rules=await M()},"Rule approved for observation mode.")}if(e.dataset.editRule){const t=e.dataset.editRule;await g(`edit-rule-${t}`,async()=>{s.selectedRule=s.rules.find(i=>i.id===t),s.selectedEmail=null,s.selectedDraft=null,s.selectedEmployee=null,s.selectedCategory=null},"Rule opened for local editing.")}if(e.dataset.saveRule){const t=e.dataset.saveRule,i=Object.fromEntries([...document.querySelectorAll("[data-rule-field]")].map(n=>[n.dataset.ruleField,n.value]));await g(`save-rule-${t}`,async()=>{await ot(t,i),s.rules=await M(),s.selectedRule=s.rules.find(n=>n.id===t)},"Rule saved locally.")}if(e.dataset.deleteRule){const t=e.dataset.deleteRule,i=s.rules.find(n=>n.id===t);s.confirmDialog={type:"delete-rule",ruleId:t,title:"Delete this rule?",message:`Delete “${(i==null?void 0:i.title)||"this rule"}” from the local demo?`,primaryLabel:"Delete rule",tone:"danger"},v()}if(e.dataset.addEmployee!==void 0&&(s.selectedEmployee={id:"new",name:"",email:"",title:"",department:""},s.selectedEmail=null,s.selectedDraft=null,s.selectedRule=null,s.selectedCategory=null,v()),e.dataset.editEmployee&&(s.selectedEmployee=s.employees.find(t=>t.id===e.dataset.editEmployee)||null,s.selectedEmail=null,s.selectedDraft=null,s.selectedRule=null,s.selectedCategory=null,v()),e.dataset.saveEmployee){const t=e.dataset.saveEmployee,i=Object.fromEntries([...document.querySelectorAll("[data-employee-field]")].map(n=>[n.dataset.employeeField,n.value]));await g(`save-employee-${t}`,async()=>{const n=t==="new"?await mt(i):await gt(t,i);s.employees=await W(),s.activity=await x(),s.selectedEmployee=s.employees.find(o=>o.id===n.id)||null},t==="new"?"Employee added locally.":"Employee changes saved locally.")}if(e.dataset.deleteEmployee){const t=e.dataset.deleteEmployee,i=s.employees.find(o=>o.id===t),n=s.emails.filter(o=>o.assignedTo===t).length;s.confirmDialog={type:"delete-employee",employeeId:t,title:"Remove this employee?",message:`Remove ${(i==null?void 0:i.name)||"this employee"}? ${n} assigned email${n===1?"":"s"} will return to Unassigned.`,primaryLabel:"Remove employee",tone:"danger"},v()}if(e.dataset.addCategory!==void 0&&(s.selectedCategory={id:"new",name:"",description:"",color:"default",active:!0,system:!1},s.selectedEmail=null,s.selectedDraft=null,s.selectedRule=null,s.selectedEmployee=null,v()),e.dataset.editCategory&&(s.selectedCategory=s.categories.find(t=>t.id===e.dataset.editCategory)||null,s.selectedEmail=null,s.selectedDraft=null,s.selectedRule=null,s.selectedEmployee=null,v()),e.dataset.saveCategory){const t=e.dataset.saveCategory,i=Object.fromEntries([...document.querySelectorAll("[data-category-field]")].map(n=>[n.dataset.categoryField,n.value]));await g(`save-category-${t}`,async()=>{const n=t==="new"?await dt(i):await ct(t,i);await se(n.id)},t==="new"?"Category added locally.":"Category changes saved locally.")}if(e.dataset.toggleCategory){const t=e.dataset.toggleCategory;await g(`toggle-category-${t}`,async()=>{const i=await ut(t);await se(i.id)},"Category visibility updated locally.")}if(e.dataset.approveDraft){const t=e.dataset.approveDraft;await g(`approve-${t}`,async()=>{await xt(t)},"Draft approved and workflow completed. Nothing was sent.")}if(e.dataset.approveSelected!==void 0&&await g("approve-selected",async()=>{await ge(s.selectedDraftIds),s.drafts=await L(),s.emails=await A(),s.activity=await x(),s.digest=await F(),s.selectedDraftIds=[]},"Selected drafts approved and workflows completed. Nothing was sent."),e.dataset.approveLowRisk!==void 0){if(!ve()){P("Low-risk bulk approval is disabled by workspace settings.",!0);return}await g("approve-low-risk",async()=>{await nt(),s.drafts=await L(),s.emails=await A(),s.activity=await x(),s.digest=await F(),s.selectedDraftIds=[]},"Low-risk drafts approved and workflows completed. Nothing was sent.")}if(e.dataset.saveSettings!==void 0){const t=Object.fromEntries([...document.querySelectorAll("[data-setting]")].map(i=>[i.dataset.setting,i.value]));await g("settings",async()=>{s.settings=await Ze(t),s.settingsForm={...s.settings}},"Settings saved locally.")}e.dataset.resetDemo!==void 0&&(s.confirmDialog={type:"reset-demo",title:"Reset demo data?",message:"This clears all local Courio changes and restores the original fake demo data.",primaryLabel:"Reset demo data",tone:"danger"},v())}});document.addEventListener("change",async a=>{const e=a.target;if(e.dataset.setting!==void 0){s.settingsForm={...B(),[e.dataset.setting]:e.value},e.dataset.setting==="mode"&&v();return}if(e.dataset.emailCategory){const t=e.dataset.emailCategory;await g(`category-${t}`,async()=>{await lt(t,e.value),await G(t)},"Category updated locally.")}if(e.dataset.emailAssignee){const t=e.dataset.emailAssignee;await g(`assign-${t}`,async()=>{await pt(t,e.value),await G(t)},"Email assignment updated locally.")}if(e.dataset.selectDraft){const t=e.dataset.selectDraft;s.selectedDraftIds=e.checked?[...new Set([...s.selectedDraftIds,t])]:s.selectedDraftIds.filter(i=>i!==t),v()}});document.addEventListener("input",a=>{const e=a.target;if(e.dataset.setting!==void 0){s.settingsForm={...B(),[e.dataset.setting]:e.value};return}if(e.dataset.ruleSearch===void 0)return;s.ruleQuery=e.value,ye();const t=document.querySelector("[data-rule-search]");t==null||t.focus(),t==null||t.setSelectionRange(s.ruleQuery.length,s.ruleQuery.length)});document.addEventListener("submit",async a=>{const e=a.target.closest(".assistant-form");if(!e)return;a.preventDefault();const t=e.querySelector("[data-assistant-input]"),i=t.value.trim();i&&(t.value="",await he(i))});v();St();
