...
`;function Z(a,e){s.busy[a]=e,w()}async function g(a,e,t){try{Z(a,!0),await e(),t&&N(t)}catch(i){N(i.message||"Something went wrong in the mock workflow.",!0)}finally{Z(a,!1)}}function u(a){return!!s.busy[a]}function N(a,e=!1){const t=document.querySelector("#toast");t.textContent=a,t.classList.toggle("error",e),t.classList.add("show"),window.clearTimeout(t.dataset.timer),t.dataset.timer=window.setTimeout(()=>t.classList.remove("show"),2400)}function x(a,e={}){if(!at.has(a))throw new Error("That Courio section is unavailable.");const t=s.tab;t==="admin"&&a!=="admin"&&(s.settingsForm=null),a==="admin"&&t!=="admin"&&(s.settingsForm={...s.settings}),s.tab=a,e.triageFilter&&(s.triageFilter=e.triageFilter),e.draftFilter&&(s.draftFilter=e.draftFilter),e.closeDrawers!==!1&&(s.selectedEmail=null,s.selectedDraft=null,s.selectedRule=null,s.selectedEmployee=null,s.summary="",s.showExplanation=!1),w()}async function W(a=(e=>(e=s.selectedEmail)==null?void 0:e.id)()){if(s.emails=await R(),a){const t=s.emails.find(i=>i.id===a);s.selectedEmail=t?{...t,messages:t.thread}:null}}function it(a){return!!(a!=null&&a.isReadyForHumanSend)}function U(){return s.settings.mode==="Advanced"}function oe(){return s.settings.allowLowRiskBulkApproval!=="No"}function r(a=""){return String(a).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function E(a=[]){return a.map(e=>r(e)).join(", ")}function j(){return s.settingsForm||{...s.settings}}function nt(){return j().mode==="Advanced"}async function ot(){var a;try{const[e,t,i,n,o,l,h,v,f]=await Promise.all([R(),P(),I(),C(),Oe(),T(),Ze(),A(),_e()]);s.emails=e,s.employees=t,s.rules=i,s.drafts=n,s.settings={...s.settings,...o},s.settingsForm=null,s.digest=l,s.assistantMessages=h,s.activity=v,s.setupPreview=f,s.selectedSetupMailboxId=((a=f.mailboxes[0])==null?void 0:a.id)||""}catch(e){N(e.message||"Could not load mock data.",!0)}finally{s.loading.emails=!1,s.loading.rules=!1,s.loading.drafts=!1,w()}}function w(){document.querySelector("#pageTitle").textContent=K[s.tab][0],document.querySelector("#pageSubtitle").textContent=K[s.tab][1],document.querySelectorAll(".section").forEach(a=>{a.classList.toggle("active",a.id===s.tab)}),document.querySelectorAll(".nav button").forEach(a=>{a.classList.toggle("active",a.dataset.tab===s.tab)}),rt(),re(),lt(),le(),vt(),ht(),dt(),ct(),wt()}function rt(){const a=s.emails.filter(t=>t.status!=="Done").length,e=s.digest;document.querySelector("#dashboard").innerHTML=`
    <div class="grid cols-3">
      ...
      <div class="panel metric">
        ...
        <div class="value">${s.drafts.filter(it).length||0}</div>
        ...
        <tbody>
          ${e.map(i=>{var n;return`
            <tr>
              ...
              <td>
                <span class="badge ${ce(i.category)}">${r(i.category)}</span><br>
  ...
  `}function dt(){const a=document.querySelector("#drawerRoot");if(s.selectedEmployee){gt(a);return}if(s.selectedRule){ft(a);return}if(s.selectedDraft){mt(a);return}if(!s.selectedEmail){a.innerHTML="";return}const e=s.selectedEmail,t=e.status==="Done",i=s.employees.find(o=>o.id===e.assignedTo),n=["Client complaint","Accounting","Sales","Documents","Missing documents","Scheduling","General"];a.innerHTML=`
    <div class="drawer-backdrop" data-close-drawer></div>
    <aside class="review-drawer" aria-label="Email review">
      <div class="drawer-header">
        <div>
          <div class="badge ${ce(e.category)}">${r(e.category)}</div>
          <h2>${r(e.subject)}</h2>
  ...
  `}function ct(){const a=document.querySelector("#modalRoot");if(!s.confirmDialog){a.innerHTML="";return}const e=s.confirmDialog;a.innerHTML=`
  ...
  `}async function ut(a){const e=await We(a);s.drafts=await C(),await W(a),s.selectedDraft=await M(e.id),s.selectedEmail=null}async function pt(a){var e;await Ve(a),s.drafts=await C(),s.emails=await R(),s.activity=await A(),s.digest=await T(),s.selectedDraftIds=s.selectedDraftIds.filter(t=>t!==a),((e=s.selectedDraft)==null?void 0:e.id)===a&&(s.selectedDraft=await M(a))}function mt(a){const e=s.selectedDraft,t=e.sourceEmail||{},i=t.status==="Done";a.innerHTML=`
    <div class="drawer-backdrop" data-close-drawer></div>
  ...
  `}function ft(a){var i;const e=s.selectedRule,t=["Client complaint","Accounting","Sales","Documents","Missing documents","Scheduling","General"];a.innerHTML=`
    <div class="drawer-backdrop" data-close-drawer></div>
  ...
  `}function gt(a){const e=s.selectedEmployee,t=e.id==="new";a.innerHTML=`
    <div class="drawer-backdrop" data-close-drawer></div>
  ...
  `}function wt(){const a=document.querySelector("#assistantRoot"),e=s.assistantMessages.length?s.assistantMessages:[{id:"assistant-loading",role:"assistant",text:"Loading assistant history..."}];a.innerHTML=`
      ...
      ${s.assistantOpen?`
        <div class="assistant-panel" aria-label="Courio assistant">
          ...
          <div class="assistant-suggestions" aria-label="Assistant command suggestions">
            ${st.map(t=>`
              <button class="assistant-chip" data-assistant-command="${r(t)}" ${u("assistant")?"disabled":""}>
                ${r(t)}
              </button>
            `).join("")}
          </div>
  ...
  `}function ce(a){return a==="Urgent"||a==="Client complaint"?"urgent":a==="Accounting"||a==="Documents"||a==="Missing documents"?"invoice":a==="Sales"?"lead":""}document.addEventListener("click",async a=>{const e=a.target.closest("button");if(e){if(e.dataset.assistantToggle!==void 0){s.assistantOpen=!s.assistantOpen,w();return}if(e.dataset.assistantCommand){await de(e.dataset.assistantCommand);return}if(e.dataset.confirmCancel!==void 0){s.confirmDialog=null,w();return}if(e.dataset.confirmPrimary!==void 0){const t=s.confirmDialog;if(s.confirmDialog=null,(t==null?void 0:t.type)==="reset-demo"){await g("reset-demo",async()=>{await He(),window.location.reload()});return}if((t==null?void 0:t.type)==="delete-rule"){await g(`delete-rule-${t.ruleId}`,async()=>{await Qe(t.ruleId),s.rules=await I(),s.activity=await A(),s.selectedRule=null},"Rule deleted locally.");return}if((t==null?void 0:t.type)==="delete-employee"){await g(`delete-employee-${t.employeeId}`,async()=>{await Xe(t.employeeId),s.employees=await P(),s.emails=await R(),s.activity=await A(),s.selectedEmployee=null},"Employee removed and assigned emails returned to Unassigned.");return}}if(e.dataset.tab&&x(e.dataset.tab),e.dataset.tabTarget&&x(e.dataset.tabTarget),e.dataset.setupMailbox&&(s.selectedSetupMailboxId=e.dataset.setupMailbox,re()),e.dataset.triageFilter&&(s.triageFilter=e.dataset.triageFilter,w()),e.dataset.draftFilter&&(s.draftFilter=e.dataset.draftFilter,w()),e.dataset.action==="digest"&&await g("digest",async()=>{s.digest=await T()},"Morning digest regenerated from local demo data."),e.dataset.reviewEmail){const t=e.dataset.reviewEmail;await g(`review-${t}`,async()=>{s.selectedEmail=await ie(t),s.emails=await R(),s.selectedDraft=null,s.selectedRule=null,s.selectedEmployee=null,s.summary="",s.showExplanation=!1},"Message thread opened.")}if(e.dataset.reviewDraft){const t=e.dataset.reviewDraft;await g(`review-draft-${t}`,async()=>{s.selectedDraft=await M(t),s.selectedEmail=null,s.selectedRule=null,s.selectedEmployee=null,s.summary="",s.showExplanation=!1},"Draft opened for review.")}if(e.dataset.openEmailDraft){const t=e.dataset.openEmailDraft;await g(`open-email-draft-${t}`,async()=>{s.selectedDraft=await Ne(t),s.selectedEmail=null,s.selectedRule=null,s.selectedEmployee=null,s.summary="",s.showExplanation=!1},"Draft opened for editing.")}if(e.dataset.closeDrawer!==void 0&&(s.selectedEmail=null,s.selectedDraft=null,s.selectedRule=null,s.selectedEmployee=null,s.summary="",s.showExplanation=!1,w()),e.dataset.toggleExplanation!==void 0&&(s.showExplanation=!s.showExplanation,w()),e.dataset.summaryEmail){const t=e.dataset.summaryEmail;await g(`summary-${t}`,async()=>{s.summary=await Pe(t)},"Thread summary generated.")}if(e.dataset.generateDraft){const t=e.dataset.generateDraft;await g(`draft-${t}`,async()=>{await ut(t)},"Draft opened. Existing edits were preserved.")}if(e.dataset.saveDraft){const t=e.dataset.saveDraft,i=document.querySelector("[data-draft-editor]");await g(`save-${t}`,async()=>{var n;await Be(t,i.value),s.drafts=await C(),s.emails=await R(),((n=s.selectedDraft)==null?void 0:n.id)===t&&(s.selectedDraft=await M(t))},"Draft saved locally.")}if(e.dataset.toggleRule){const t=e.dataset.toggleRule;await g(`rule-${t}`,async()=>{await X(t),s.rules=await I()},"Rule preview state updated.")}if(e.dataset.approveRule){const t=e.dataset.approveRule;await g(`approve-rule-${t}`,async()=>{s.rules.find(n=>n.id===t).on||await X(t),s.rules=await I()},"Rule approved for observation mode.")}if(e.dataset.editRule){const t=e.dataset.editRule;await g(`edit-rule-${t}`,async()=>{s.selectedRule=s.rules.find(i=>i.id===t),s.selectedEmail=null,s.selectedDraft=null,s.selectedEmployee=null},"Rule opened for local editing.")}if(e.dataset.saveRule){const t=e.dataset.saveRule,i=Object.fromEntries([...document.querySelectorAll("[data-rule-field]")].map(n=>[n.dataset.ruleField,n.value]));await g(`save-rule-${t}`,async()=>{await ze(t,i),s.rules=await I(),s.selectedRule=s.rules.find(n=>n.id===t)},"Rule saved locally.")}if(e.dataset.deleteRule){const t=e.dataset.deleteRule,i=s.rules.find(n=>n.id===t);s.confirmDialog={type:"delete-rule",ruleId:t,title:"Delete this rule?",message:`Delete “${(i==null?void 0:i.title)||"this rule"}” from the local demo?`,primaryLabel:"Delete rule",tone:"danger"},w()}if(e.dataset.addEmployee!==void 0&&(s.selectedEmployee={id:"new",name:"",email:"",title:"",department:""},s.selectedEmail=null,s.selectedDraft=null,s.selectedRule=null,w()),e.dataset.editEmployee&&(s.selectedEmployee=s.employees.find(t=>t.id===e.dataset.editEmployee)||null,s.selectedEmail=null,s.selectedDraft=null,s.selectedRule=null,w()),e.dataset.saveEmployee){const t=e.dataset.saveEmployee,i=Object.fromEntries([...document.querySelectorAll("[data-employee-field]")].map(n=>[n.dataset.employeeField,n.value]));await g(`save-employee-${t}`,async()=>{const n=t==="new"?await Ke(i):await Je(t,i);s.employees=await P(),s.activity=await A(),s.selectedEmployee=s.employees.find(o=>o.id===n.id)||null},t==="new"?"Employee added locally.":"Employee changes saved locally.")}if(e.dataset.deleteEmployee){const t=e.dataset.deleteEmployee,i=s.employees.find(o=>o.id===t),n=s.emails.filter(o=>o.assignedTo===t).length;s.confirmDialog={type:"delete-employee",employeeId:t,title:"Remove this employee?",message:`Remove ${(i==null?void 0:i.name)||"this employee"}? ${n} assigned email${n===1?"":"s"} will return to Unassigned.`,primaryLabel:"Remove employee",tone:"danger"},w()}if(e.dataset.approveDraft){const t=e.dataset.approveDraft;await g(`approve-${t}`,async()=>{await pt(t)},"Draft approved and workflow completed. Nothing was sent.")}if(e.dataset.approveSelected!==void 0&&await g("approve-selected",async()=>{await ne(s.selectedDraftIds),s.drafts=await C(),s.emails=await R(),s.activity=await A(),s.digest=await T(),s.selectedDraftIds=[]},"Selected drafts approved and workflows completed. Nothing was sent."),e.dataset.approveLowRisk!==void 0){if(!oe()){N("Low-risk bulk approval is disabled by workspace settings.",!0);return}await g("approve-low-risk",async()=>{await Ge(),s.drafts=await C(),s.emails=await R(),s.activity=await A(),s.digest=await T(),s.selectedDraftIds=[]},"Low-risk drafts approved and workflows completed. Nothing was sent.")}if(e.dataset.saveSettings!==void 0){const t=Object.fromEntries([...document.querySelectorAll("[data-setting]")].map(i=>[i.dataset.setting,i.value]));await g("settings",async()=>{s.settings=await je(t),s.settingsForm={...s.settings}},"Settings saved locally.")}e.dataset.resetDemo!==void 0&&(s.confirmDialog={type:"reset-demo",title:"Reset demo data?",message:"This clears all local Courio changes and restores the original fake demo data.",primaryLabel:"Reset demo data",tone:"danger"},w())}});document.addEventListener("change",async a=>{const e=a.target;if(e.dataset.setting!==void 0){s.settingsForm={...j(),[e.dataset.setting]:e.value},e.dataset.setting==="mode"&&w();return}if(e.dataset.emailCategory){const t=e.dataset.emailCategory;await g(`category-${t}`,async()=>{await Ue(t,e.value),await W(t)},"Category updated locally.")}if(e.dataset.emailAssignee){const t=e.dataset.emailAssignee;await g(`assign-${t}`,async()=>{await Ye(t,e.value),await W(t)},"Email assignment updated locally.")}if(e.dataset.selectDraft){const t=e.dataset.selectDraft;s.selectedDraftIds=e.checked?[...new Set([...s.selectedDraftIds,t])]:s.selectedDraftIds.filter(i=>i!==t),w()}});document.addEventListener("input",a=>{const e=a.target;if(e.dataset.setting!==void 0){s.settingsForm={...j(),[e.dataset.setting]:e.value};return}if(e.dataset.ruleSearch===void 0)return;s.ruleQuery=e.value,le();const t=document.querySelector("[data-rule-search]");t==null||t.focus(),t==null||t.setSelectionRange(s.ruleQuery.length,s.ruleQuery.length)});document.addEventListener("submit",async a=>{const e=a.target.closest(".assistant-form");if(!e)return;a.preventDefault();const t=e.querySelector("[data-assistant-input]"),i=t.value.trim();i&&(t.value="",await de(i))});w();ot();
